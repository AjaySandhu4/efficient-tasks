...
export default class CompositionsCourseListCourseModalComponent extends Component<Args> {
  ...
  @tracked courseCode?: string = this.args.course ? this.args.course.code : undefined;
  @tracked courseColor?: number = this.args.course ? this.args.course.color : undefined;

  get isDoneButtonDisabled(): boolean {
    if(!this.courseCode || !this.courseColor) return true;
    else if(this.courseCode === this.args.course?.code && this.courseColor === this.args.course?.color) return true;
    else if(this.courseCode !== this.args.course?.code && !this.validateCode(this.courseCode)) return true;
    return false; 
  }

  @action async submit(): Promise<void> {
    if(!this.args.course && this.courseColor && this.courseCode){ //Adding a new course
        await this.firestore.addCourse({code: this.courseCode, color: this.courseColor});
    }
  ...
  validateCode(code: string): boolean {
    console.log(this.firestore.currSchedule)
