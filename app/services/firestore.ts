...
export default class FirestoreService extends Service {
    ...
    onAuthStateChanged(getAuth(this.app), (user) => {
      if (user) {
        ...
        getDoc(docRef).then((docSnap) => {
          if (!docSnap.exists()) {
            setDoc(docRef, { name: user.displayName, email: user.email }).catch(
              (e) => console.log(e)
            );
          }
          this.user = {
  ...
  @action async fetchTasks(scheduleId: string): Promise<void> {
    ...
    const colRef = collection(
      this.db,
      `users/${this.user.id}/schedules/${scheduleId}`,
      'tasks'
    );
    await getDocs(colRef).then((querySnap: QuerySnapshot) => {
      querySnap.forEach((doc) => {
  ...
  @action async updateScheduleName(newName: string): Promise<void> {
    if (
      !this.user ||
      !this.currSchedule ||
      !this.schedules[this.currSchedule.id]
    )
      return;
    const docRef = doc(
      this.db,
      `users/${this.user.id}/schedules`,
      this.currSchedule.id
    );
  ...
  @action async updateCourse(
    ...
    const tasksRef = collection(
      this.db,
      `users/${this.user.id}/schedules/${this.currSchedule.id}/tasks`
    );
    const taskQuery = query(
      tasksRef,
      where('courseCode', '==', originalCourse.code)
    );
    ...
    let courseToUpdateIndex: number = this.currSchedule.courses.findIndex(
      (c) => c.code === originalCourse.code
    );
    if (courseToUpdateIndex != -1)
      set(this.currSchedule.courses, courseToUpdateIndex, updatedCourse);
  ...
  @action async addTask(newTask: Task): Promise<void> {
    ...
    const colRef = collection(
      this.db,
      `users/${this.user.id}/schedules/${this.currSchedule.id}`,
      'tasks'
    );
  ...
  @action async deleteCourse(course: Course): Promise<void> {
    ...
    this.currSchedule.courses = this.currSchedule.courses.filter(
      (c) => c.code !== course.code
    );
    Object.values(this.currSchedule.tasks).forEach((t) => {
      if (t.courseCode === course.code && this.currSchedule)
        delete this.currSchedule.tasks[t.id];
    });
    this.currSchedule = this.currSchedule;
  }
